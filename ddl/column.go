...
func (w *worker) onModifyColumn(d *ddlCtx, t *meta.Meta, job *model.Job) (ver int64, _ error) {
	...
	if oldCol == nil || oldCol.State != model.StatePublic {
		job.State = model.JobStateCancelled
		return ver, infoschema.ErrColumnNotExists.GenWithStackByArgs(oldColName, tblInfo.Name)
	}
	...
	if updatedAutoRandomBits > 0 {
		if err := checkAndApplyNewAutoRandomBits(job, t, tblInfo, newCol, oldColName, updatedAutoRandomBits); err != nil {
			return ver, errors.Trace(err)
		}
	}
	...
	if changingCol == nil {
		...
		// TODO: check primary key flag.
		// TODO: Check whether we need to check OriginDefaultValue.
		changingCol = newCol.Clone()
		changingCol.Name = newColName
		changingCol.ChangeStateInfo = &model.ChangeStateInfo{RelativeColumnOffset: oldCol.Offset}
		...
		changingIdxs = make([]*model.IndexInfo, 0, len(idxInfos))
		...
		//		changingCol = model.FindColumnInfo(tblInfo.Columns, changingCol.Name.L)
		//		if changingCol != nil {
		//			if changingCol.State == model.StatePublic {
		//				job.State = model.JobStateCancelled
		//				return 0, infoschema.ErrColumnExists.GenWithStackByArgs(changingCol.Name)
		//			}
		//		}
	}
...
func (w *worker) doModifyColumnType(
	...
	case model.StateWriteReorganization:
		...
		tblInfo.Columns = tblInfo.Columns[:len(tblInfo.Columns)-1]
		idxInfos, _ := findIndexesByColName(tblInfo.Indices, oldCol.Name.L)
		for i, idx := range tblInfo.Indices {
			for _, info := range idxInfos {
				if idx.Name.L != info.Name.L {
					continue
				}
				for _, cIdx := range changingIdxs {
					if d := strings.TrimPrefix(cIdx.Name.O, changingIndexPrefix); d == cIdx.Name.O {
		...
		// changingCol.Hidden = false
		tblInfo.Indices = tblInfo.Indices[:len(tblInfo.Indices)-len(idxInfos)]
		// Adjust table column offset.
