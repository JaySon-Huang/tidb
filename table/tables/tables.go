	...
	for _, col := range t.WritableCols() {
		...
		if col.State != model.StatePublic {
			...
			if col.ChangeStateInfo != nil {
				...
				value, err = table.CastValue(sctx, newData[col.RelativeColumnOffset], col.ColumnInfo, false, false)
				if err != nil {
					return err
				}
			}
...
func (t *TableCommon) AddRecord(ctx sessionctx.Context, r []types.Datum, opts ...table.AddRecordOption) (recordID kv.Handle, err error) {
	...
	for _, col := range t.WritableCols() {
		...
		if col.State != model.StatePublic && col.ChangeStateInfo != nil {
			// TODO: Check overflow or ignoreTruncate.
			value, err = table.CastValue(ctx, r[col.RelativeColumnOffset], col.ColumnInfo, false, false)
			if err != nil {
				return recordID, err
			}
